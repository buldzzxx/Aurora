...
void Client::DrawHUD() {
	...
	std::stringstream ss;
	ss << tfm::format(XOR("aurora | fps: %i | latency: %i ms"), fps, ms);
	...
	// line watermark
	render::rect_filled(m_width - size.m_width - 20, 28, size.m_width + 12, 2, { g_gui.m_color });
...
void Client::ClanTag()
{
	...
	if (g_menu.main.misc.clantag.get()) {
		...
		if (iPrevFrame != int(g_csgo.m_globals->m_curtime * 2.9) % 7) {
			...
			case 2: {  SetClanTag("<-u---->"); break; }
			case 3: {  SetClanTag("<--r--->"); break; }
			case 4: {  SetClanTag("<---o-->"); break; }
			case 5: {  SetClanTag("<----r->"); break; }
			case 6: {  SetClanTag("<-----a>"); break; }
...
void Client::ClanTag2()
{
	...
	if (g_menu.main.misc.clantag2.get()) {
		...
		if (iPrevFrame != int(g_csgo.m_globals->m_curtime * 2.5) % 6) {
			switch (int(g_csgo.m_globals->m_curtime * 2.5) % 6) {
			case 0: {  SetClanTag("<------>"); break; }
			case 1: {  SetClanTag("<--ro-->"); break; }
			case 2: {  SetClanTag("<au--ra>"); break; }
			case 3: {  SetClanTag("<u----a>"); break; }
			case 4: {  SetClanTag("<------>"); break; }
			case 5: {  SetClanTag("<au--ra>"); break; }
			case 6: {  SetClanTag("<aurora>"); break; }

			default:;
			}
			iPrevFrame = int(g_csgo.m_globals->m_curtime * 2.5) % 6;
		}
...
void Client::print(const std::string text, ...) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf(colors::yellowgreen, XOR("[Aurora] "));
